...
describe("Enhanced Cache with Persistent Storage", () => {
  ...
  test("should persist values to localStorage", () => {
    ...
    expect(Object.keys(localStorageMock.getAllItems()).length).toBeGreaterThan(
      0,
    );

    // Create a new cache instance (simulating page reload)
    const newCache = new (cache.constructor as new (
  ...
  test("should delete values", () => {
    ...
    const storageKey = Object.keys(localStorageMock.getAllItems()).find((k) =>
      k.includes(key),
    );
