 ...
 */
export function getBaselineQuality(
  baseline: UserBaseline | null,
): BaselineQuality {
 ...
 */
export function getBaselineQualityDescription(
 ...
 * @returns A human-readable description
 */
export function getCalculationMethodDescription(
  method: BaselineCalculationMethod,
): string {
  switch (method) {
    case "first_n_tests":
...
export function getBaselineRecommendations(
  ...
  if (
    ...
    recommendations.push(
      "Take more cognitive tests to improve baseline accuracy",
    );

    if (baseline.sampleSize < 3) {
      recommendations.push(
        "At least 3 tests are recommended for a reliable baseline",
      );
    }

    if (baseline.varianceScore && baseline.varianceScore > 100) {
      recommendations.push(
        "Try to maintain consistent testing conditions to reduce variance",
      );
    }
  }
  ...
  if (quality === BaselineQuality.MODERATE && baseline.sampleSize < 5) {
    recommendations.push(
      "Take a few more tests to further improve baseline accuracy",
    );
  }
