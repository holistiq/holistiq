...
export function SupplementEvaluationStatus({
  ...
  useEffect(() => {
    ...
    const beforeTests = testResults.filter(test => 
    ...
    const afterTests = testResults.filter(test => 
      isAfter(parseISO(test.timestamp), startDate) && 
      (!supplement.end_date || isBefore(parseISO(test.timestamp), parseISO(supplement.end_date)))
    );
