...
export function BadgeSelector({
  ...
  const isAchievementSelected = (achievementId: string) => {
    return selectedBadges.some(badge => badge.achievementId === achievementId);
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Select up to {MAX_DISPLAYED_BADGES} achievements to display as badges on your profile
          </DialogDescription>
        </DialogHeader>
          ...
          {selectedBadges.length === 0 ? (
            <div className="text-center py-6 border border-dashed rounded-md">
              ...
              <p className="text-sm text-muted-foreground">
                No badges selected yet. Choose from your completed achievements below.
              </p>
            </div>
          ) : (
            ...
            <ScrollArea className="h-[300px] pr-4">
              <div className="space-y-3">
                {completedAchievements.map(achievement => (
                  <div key={achievement.id} className="flex items-center justify-between border rounded-md p-3">
                    ...
                    <Button
                      variant={isAchievementSelected(achievement.id) ? "secondary" : "outline"}
                      size="sm"
                      className="gap-1"
