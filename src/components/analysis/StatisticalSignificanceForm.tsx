...
export function StatisticalSignificanceForm({
  ...
  const [baselineStart, setBaselineStart] = useState<Date | undefined>(
    subDays(new Date(), 30),
  );
  const [baselineEnd, setBaselineEnd] = useState<Date | undefined>(
    subDays(new Date(), 16),
  );
  const [comparisonStart, setComparisonStart] = useState<Date | undefined>(
    subDays(new Date(), 15),
  );
  const [comparisonEnd, setComparisonEnd] = useState<Date | undefined>(
    new Date(),
  );
  ...
  const handleSubmit = (e: React.FormEvent): void => {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Please ensure dates are in chronological order: baseline start → baseline end → comparison start → comparison end.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Compare two time periods to determine if changes in cognitive
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value={ContextType.GENERAL}>
                  General Analysis
                </SelectItem>
                <SelectItem value={ContextType.SUPPLEMENT}>
                  Supplement Analysis
                </SelectItem>
                <SelectItem value={ContextType.CONFOUNDING_FACTOR}>
            ...
            <div className="space-y-2">
              ...
              <Select value={contextId} onValueChange={setContextId}>
                <SelectTrigger id="context-id">
                  <SelectValue
                    placeholder={`Select ${contextType === ContextType.SUPPLEMENT ? "supplement" : "factor"}`}
                  />
                </SelectTrigger>
                <SelectContent>
