...
export function FactorTimeline({
  ...
  const getFactorValue = (factor: ConfoundingFactor) => {
    switch (factorType) {
      case 'sleep':
        ...
        return 'Recorded';
      case 'stress':
        return factor.stress_level ? `Level: ${factor.stress_level}/10` : 'Recorded';
      case 'exercise':
        ...
        return 'Recorded';
      case 'caffeine':
        return factor.caffeine_intake ? `${factor.caffeine_intake} mg` : 'Recorded';
      case 'alcohol':
        return factor.alcohol_intake ? `${factor.alcohol_intake} drinks` : 'Recorded';
      case 'mood':
        return factor.mood ? `Level: ${factor.mood}/10` : 'Recorded';
      case 'energy':
        return factor.energy_level ? `Level: ${factor.energy_level}/10` : 'Recorded';
      default:
        return 'Recorded';
    }
  };
  ...
  const rangeStart = dateRange.from ? dateRange.from.getTime() : 
    Math.min(...filteredFactors.map(f => new Date(f.recorded_at).getTime()));
  
  const rangeEnd = dateRange.to ? dateRange.to.getTime() : 
    Math.max(...filteredFactors.map(f => new Date(f.recorded_at).getTime()));
