    ...
    <div className="bg-background border rounded-md shadow-md p-3 max-w-[250px]">
      ...
      {originalDateString && (
        <p className="text-xs text-muted-foreground mb-2">
          Actual time: {originalDateString}
        </p>
      )}
          ...
          <p className="text-sm flex justify-between">
            ...
            <span className="font-medium">
              ...
              {data.scoreTrend !== undefined && data.scoreTrend !== 0 && (
                ...
                >
                  {data.scoreTrend > 0 ? "↑" : "↓"}{" "}
                  {Math.abs(data.scoreTrend).toFixed(1)}%
                </span>
              )}
            </span>
          </p>
        ...
        {data.reactionTime !== undefined && (
          <p className="text-sm flex justify-between">
            ...
            <span className="font-medium">
              ...
              {data.reactionTimeTrend !== undefined &&
                data.reactionTimeTrend !== 0 && (
                  <span
                    className={
                      data.reactionTimeTrend > 0
                        ? "text-green-500 ml-1"
                  ...
                  >
                    {data.reactionTimeTrend > 0 ? "↑" : "↓"}{" "}
                    {Math.abs(data.reactionTimeTrend).toFixed(1)}%
                  </span>
                )}
            </span>
          </p>
          ...
          <p className="text-sm flex justify-between">
            ...
            <span className="font-medium">
              ...
              {data.accuracyTrend !== undefined && data.accuracyTrend !== 0 && (
                ...
                >
                  {data.accuracyTrend > 0 ? "↑" : "↓"}{" "}
                  {Math.abs(data.accuracyTrend).toFixed(1)}%
                </span>
              )}
            </span>
          </p>
