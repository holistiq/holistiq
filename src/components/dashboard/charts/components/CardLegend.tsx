...
export function CardLegend({
  ...
  const renderLegendItem = (item: { label: string; color: string; isDashed: boolean; type: string }) => {
    ...
    if (item.type === 'baseline') {
      ...
      lineStyle = (
        <div className="flex items-center">
          <div className="w-12 border-t-2 border-dashed h-0 flex-shrink-0"
      ...
      lineStyle = (
        <div className="flex items-center">
          <div className="w-12 border-t-2 border-dotted h-0 flex-shrink-0"
               style={{ borderColor: item.color }} />
        </div>
      );
      dotStyle = (
        <div className="w-3 h-3 rounded-full border-2 flex-shrink-0 bg-white"
             style={{ borderColor: item.color }} />
      );
    } else {
      ...
      lineStyle = (
        <div className="flex items-center">
          <div className="w-12 h-1.5 flex-shrink-0"
               style={{ backgroundColor: item.color }} />
        </div>
      );
      dotStyle = (
        <div className="w-4 h-4 rounded-full border-2 flex-shrink-0 bg-white"
             style={{ borderColor: item.color }} />
      );
    }
  ...
  const renderMetricCard = (
    title: string,
    items: Array<{ label: string; color: string; isDashed: boolean; type: string }>,
    color: string
  ) => (
    <div
  ...
  const renderCards = () => {
    if (mode === 'single') {
      ...
      switch (dataKey) {
        ...
        case 'reactionTime':
          return renderMetricCard('Reaction Time', reactionTimeItems, chartConfig.reactionTime.color);
        case 'accuracy':
          return renderMetricCard('Accuracy', accuracyItems, chartConfig.accuracy.color);
        default:
          return null;
      }
    } else {
      ...
      return (
        <>
          ...
          {renderMetricCard('Reaction Time', reactionTimeItems, chartConfig.reactionTime.color)}
          {renderMetricCard('Accuracy', accuracyItems, chartConfig.accuracy.color)}
        </>
      );
    }
  };
  ...
  return (
    <div className="relative mt-8 pt-4 performance-chart-legend"> {/* Added class for responsive styling */}
      {/* Visual connector line */}
      <div className="absolute left-1/2 transform -translate-x-1/2 w-0.5 h-8 bg-gray-200 top-0"></div>

      {/* Legend cards container */}
      <div className={`flex flex-wrap ${mode === 'single' ? 'justify-center' : 'gap-8 justify-center'} mt-4 mb-6 px-4`}>
        {renderCards()}
      </div>
    </div>
  );
