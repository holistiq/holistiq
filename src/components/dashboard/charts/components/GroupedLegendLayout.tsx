...
export function GroupedLegendLayout({
  ...
  const reactionTimeItems = legendItems.filter(
    (item) => item.metricKey === "reactionTime",
  );
  const accuracyItems = legendItems.filter(
    (item) => item.metricKey === "accuracy",
  );
  ...
  const renderGroup = (title: string, items: LegendItem[], color: string) => {
    ...
    return (
      <div
        className="flex flex-col items-center border rounded-md shadow-sm bg-card m-2 overflow-hidden min-w-[180px] flex-1 max-w-[250px]"
        style={{ borderColor: `${color}40` }}
      >
        {/* Card header with colored background */}
        <div
          className="w-full text-center py-1.5 px-4 font-medium text-sm text-white"
  ...
  return (
    <div
      className={cn("flex flex-wrap justify-center gap-2 mt-4 mb-2", className)}
    >
