...
export function SessionProvider({ children }: Readonly<SessionProviderProps>) {
   ...
   */
  const trySetSessionFromDirectTokens = useCallback(
    async (directSession: SupabaseSession): Promise<boolean> => {
      ...
      try {
        ...
        console.error(
          "Error setting session from direct tokens:",
          setSessionError,
        );
        return false;
      }
  ...
  useEffect(() => {
    const initializeWithFallback = async () => {
      try {
        ...
        const isSignInPage =
          currentPath === "/signin" || currentPath === "/login";
        ...
        if (!user && !isOAuthCallback && !isSignInPage && !isJustSignedIn) {
          console.log(
            "No user found and not in authentication flow - checking for recoverable session...",
          );
          ...
          if (directSession) {
            ...
            if (directUser) {
              console.log(
                "Found recoverable session for user:",
                directUser.email,
              );
              const recovered =
          ...
          console.log(
            "Skipping session recovery - user just completed sign-in",
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (user && initialized) {
      ...
      prefetchService.prefetch(user.id).catch(() => {
