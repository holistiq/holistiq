...
export async function getConfoundingFactors(
  ...
  try {
    debugLog(
      `Fetching confounding factors for user (ID: ${userId.substring(0, 8)}...)`,
    );
      ...
      async () => {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            if (error) {
              debugError(
                `Error fetching confounding factors (attempt ${retryCount + 1}):`,
                error,
              );
              lastError = error;
              retryCount++;
            ...
            debugLog(
              `Retrieved ${data?.length || 0} confounding factors from Supabase`,
            );
            ...
            debugError(
              `Unexpected error fetching confounding factors (attempt ${retryCount + 1}):`,
              error,
            );
            lastError = error;
            retryCount++;
        ...
        return {
          ...
          error:
            lastError instanceof Error
              ? lastError.message
              : "Failed to fetch confounding factors after multiple attempts",
        };
      },
...
export async function analyzeConfoundingFactors(
  ...
  try {
    debugLog(
      `Analyzing confounding factors for user (ID: ${userId.substring(0, 8)}...), test type ${testType}`,
    );

    // Call the Supabase function to analyze correlations
    const { data, error } = await supabase.rpc("analyze_confounding_factors", {
      p_user_id: userId,
...
export async function getTestConfoundingFactors(
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_test_confounding_factors", {
      p_user_id: userId,
      p_test_id: testId,
    });
