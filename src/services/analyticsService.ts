...
export async function calculateAnalytics(
  ...
  try {
    console.log(
      `Calculating analytics for user ${userId}, test type ${options.testType}`,
    );

    // Call the Supabase function to calculate analytics
    const { data, error } = await supabase.rpc("calculate_analytics", {
      p_user_id: userId,
...
export async function convertAnalyticsToStatisticalAnalyses(
  ...
  try {
    console.log(
      `Converting analytics data to statistical analyses with contextType: ${contextType || "all"}`,
    );
    ...
    Object.keys(groupedByTestType).forEach((testType) => {
      ...
      const significantChanges = testTypeData.filter(
        (item) =>
    ...
    if (contextType) {
      ...
      processedAnalytics.forEach((item, index) => {
        analyses.push(
          createAnalysisFromAnalytics(item, contextType, contextId),
        );
      });

      console.log(
        `Created ${analyses.length} analyses with context type: ${contextType}`,
      );
    } else {
      ...
      processedAnalytics.forEach((item, index) => {
        ...
        analyses.push(
          createAnalysisFromAnalytics(item, selectedType, contextId),
        );
      ...
      processedAnalytics
        ...
        .forEach((item, index) => {
          ...
          analyses.push(
            createAnalysisFromAnalytics(item, itemContextType, contextId),
          );
        });

      console.log(
        `Created mixed analyses with counts: General=${typeCount[ContextType.GENERAL]}, Supplement=${typeCount[ContextType.SUPPLEMENT]}, Factor=${typeCount[ContextType.CONFOUNDING_FACTOR]}`,
      );
    }
    ...
    console.log(
      `Created ${limitedAnalyses.length} statistical analyses (context type: ${contextType || "all"})`,
    );
...
function createAnalysisFromAnalytics(
  ...
  return {
    ...
    baseline_period_start: new Date(
      new Date(item.period_start).getTime() - 30 * 24 * 60 * 60 * 1000,
    ).toISOString(), // 30 days before period as baseline
    ...
    results: {
      ...
      significance_analysis: {
        score: {
          ...
          effect_size_interpretation: interpretEffectSize(
            Math.abs(item.score_percent_change) / 100,
          ),
        ...
        accuracy: {
          ...
          effect_size_interpretation: interpretEffectSize(
            Math.abs(item.accuracy_percent_change) / 100,
          ),
...
function generateContextName(
  ...
  const generator =
    nameGenerators[contextType] || nameGenerators[ContextType.GENERAL];
