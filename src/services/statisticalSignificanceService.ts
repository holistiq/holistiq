...
import { getAnalytics, convertAnalyticsToStatisticalAnalyses } from '@/services/analyticsService';
...
export async function runStatisticalAnalysis(
  ...
  try {
    console.log(`Running statistical analysis for user ${userId}, test type ${options.testType}`);
...
export async function getStatisticalAnalysis(
  ...
  try {
    console.log(`Fetching statistical analysis ${analysisId} for user ${userId}`);
    ...
    return await cache.getOrSet(
      cacheKey,
      async () => {
        console.log('Cache miss for statistical analysis, fetching from Supabase');
...
export async function getStatisticalAnalyses(
  ...
  try {
    console.log(`Fetching statistical analyses for user ${userId}, context type: ${contextType || 'all'}, context ID: ${contextId || 'none'}`);
    ...
    if (contextType) {
      cache.delete(new RegExp(`^statistical_analyses_${userId}_(?!${contextTypeStr})`));
    }

    return await cache.getOrSet(
      cacheKey,
      async () => {
        console.log('Cache miss for statistical analyses, fetching from Supabase');
        ...
        try {
          ...
          if (!error && data && data.length > 0) {
            console.log(`Retrieved ${data.length} statistical analyses from Supabase`);
            return {
              success: true,
          ...
          console.log('Error fetching from statistical_analyses table, will try analytics data instead:', error);
        }
        ...
        console.log(`Passing context type to convertAnalyticsToStatisticalAnalyses: ${contextType || 'all'}`);
        return await convertAnalyticsToStatisticalAnalyses(analyticsResult.data, contextType, contextId, 15);
      },
...
export async function deleteStatisticalAnalysis(
  ...
  try {
    console.log(`Deleting statistical analysis ${analysisId} for user ${userId}`);
