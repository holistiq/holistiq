...
export async function saveTestResult(
  ...
  try {
    console.log(
      `Saving test result for user ${userId}, test type ${testType}, isBaseline: ${isBaseline}`,
    );
    ...
    if (userId) {
      ...
      console.log(
        "User not logged in, test result saved to local storage only",
      );
    }
      ...
      async () => {
        ...
        while (retryCount < maxRetries) {
          try {
            ...
            if (error) {
              console.error(
                `Error fetching test results (attempt ${retryCount + 1}):`,
                error,
              );
              lastError = error;
              retryCount++;
            ...
            console.log(
              `Retrieved ${data?.length || 0} test results from Supabase`,
            );
            ...
            if (data && data.length > 0) {
              ...
              if (timestamps.length !== uniqueTimestamps.size) {
                console.warn(
                  `Found ${timestamps.length - uniqueTimestamps.size} duplicate timestamps in Supabase results`,
                );
              }
            } else {
            ...
            console.error(
              `Unexpected error fetching test results (attempt ${retryCount + 1}):`,
              error,
            );
            lastError = error;
            retryCount++;
...
export async function getBaselineResult(userId: string, testType: string) {
  try {
    console.log(
      `Fetching baseline result for user ${userId} and test type ${testType}`,
    );
    ...
    return await supabaseCache.query(
      ...
      async () => {
        ...
        console.log(
          `Baseline result found: ${data && data.length > 0 ? "Yes" : "No"}`,
        );
        if (data && data.length > 0) {
        ...
        console.log(
          "No baseline found, checking for any test result as fallback",
        );
        ...
        console.log(
          `Any test result found: ${anyData && anyData.length > 0 ? "Yes" : "No"}`,
        );
...
export async function linkTestWithConfoundingFactors(
  ...
  try {
    console.log(
      `Linking test ${testId} with confounding factors ${confoundingFactorId}`,
    );

    // Call the Supabase function to link the test with confounding factors
    const { data, error } = await supabase.rpc("link_test_confounding_factor", {
      p_test_id: testId,
    ...
    console.error(
      "Unexpected error linking test with confounding factors:",
      error,
    );
    return {
      success: false,
...
export async function getTestsWithoutConfoundingFactors(
  ...
  try {
    console.log(
      `Fetching tests without confounding factors for user ${userId}`,
    );

    // Use the cache key pattern from our configuration
    const cacheKey =
      CACHE_CONFIG.TEST_RESULTS.PATTERNS.WITHOUT_CONFOUNDING(userId);
    ...
    return await supabaseCache.query(
      ...
      async () => {
        console.log(
          "Cache miss for tests without confounding factors, fetching from Supabase",
        );

        // Call the Supabase function to get tests without confounding factors
        const { data, error } = await supabase.rpc(
          "get_tests_without_confounding_factors",
          {
            p_user_id: userId,
            p_limit: limit,
          },
        ...
        if (error) {
          console.error(
            "Error fetching tests without confounding factors:",
            error,
          );
          return { success: false, error: error.message };
        }
        ...
        console.log(
          `Retrieved ${data?.length ?? 0} tests without confounding factors`,
        );
        return { success: true, data };
      },
      CACHE_CONFIG.TEST_RESULTS.TTL,
    ...
    console.error(
      "Unexpected error fetching tests without confounding factors:",
      error,
    );
    return {
      success: false,
