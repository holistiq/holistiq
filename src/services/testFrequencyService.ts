 ...
 */
export async function checkTestFrequency(
  userId: string,
): Promise<TestFrequencyStatus> {
  try {
    // Default response
    ...
    const inCooldown =
      hoursSinceLastTest < TEST_FREQUENCY.MIN_HOURS_BETWEEN_TESTS;
    ...
    const testsToday = testDates.filter(
      (date) => differenceInDays(date, today) === 0,
    ).length;

    // Calculate tests taken this week
    const testsThisWeek = testDates.filter(
      (date) => differenceInDays(now, date) < 7,
    ).length;

    // Calculate remaining tests
    const testsRemainingToday = Math.max(
      0,
      TEST_FREQUENCY.MAX_TESTS_PER_DAY - testsToday,
    );
    const testsRemainingThisWeek = Math.max(
      0,
      TEST_FREQUENCY.MAX_TESTS_PER_WEEK - testsThisWeek,
    );

    // Calculate next recommended test time
    const nextRecommendedTestTime = new Date(lastTestTime);
    nextRecommendedTestTime.setHours(
      nextRecommendedTestTime.getHours() +
        TEST_FREQUENCY.RECOMMENDED_DAYS_BETWEEN_TESTS * 24,
    );

    // Determine if user can take a test
    const canTakeTest =
      !inCooldown && testsRemainingToday > 0 && testsRemainingThisWeek > 0;
    ...
    if (inCooldown) {
      ...
      message =
        "You can take a test now, but waiting longer between tests provides more meaningful results.";
    }
...
export async function processDailyTests(
  ...
  try {
    ...
    const totalReactionTime = testsForDay.reduce(
      (sum, test) => sum + test.reaction_time,
      0,
    );
    const totalAccuracy = testsForDay.reduce(
