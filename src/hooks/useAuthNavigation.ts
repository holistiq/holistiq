...
export function useAuthNavigation() {
  ...
  useEffect(() => {
    ...
    // Add event listeners
    window.addEventListener(AUTH_EVENTS.SIGNED_OUT, handleSignOut as EventListener);
    window.addEventListener(AUTH_EVENTS.SESSION_EXPIRED, handleSessionExpired as EventListener);

    // Clean up event listeners
    return () => {
      window.removeEventListener(AUTH_EVENTS.SIGNED_OUT, handleSignOut as EventListener);
