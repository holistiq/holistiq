...
export default function SupplementEffectivenessReports() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (testResultsResult.success && testResultsResult.data) {
        ...
        if (testResultsResult.error && !testResultsResult.error.includes("no rows")) {
          toast({
            title: 'Error loading test results',
            description: testResultsResult.error ?? 'Failed to load test results',
            variant: 'destructive'
          });
        } else {
          console.log('No test results found or expected error:', testResultsResult.error);
        }
      }
      ...
      if (correlationsResult.success) {
        ...
        // Only show error toast if it's not an expected error like "no rows"
        if (correlationsResult.error && !correlationsResult.error.includes("no rows")) {
          toast({
            title: 'Error loading correlations',
            description: correlationsResult.error ?? 'Failed to load correlations',
            variant: 'destructive'
          });
        } else {
          console.log('No correlations found or expected error:', correlationsResult.error);
        }
      }
      ...
      if (analysesResult.success) {
        ...
        if (analysesResult.error && !analysesResult.error.includes("no rows")) {
          toast({
            ...
            description: analysesResult.error ?? 'Failed to load statistical analyses',
            variant: 'destructive'
          });
        } else {
          console.log('No statistical analyses found or expected error:', analysesResult.error);
        }
      }
    } catch (error) {
  ...
  const filterByTab = (supplement: Supplement, tab: string, correlations: SupplementCorrelation[]): boolean => {
    const supplementCorrelations = correlations.filter(c => c.supplement_id === supplement.id);
    ...
    const overallImpact = (scoreImpact + normalizedReactionTimeImpact + accuracyImpact) / 3;
    ...
    switch (tab) {
      case 'positive':
        return significance === ImpactSignificance.POSITIVE ||
               significance === ImpactSignificance.VERY_POSITIVE;
      case 'neutral':
        return significance === ImpactSignificance.NEUTRAL;
      case 'negative':
        return significance === ImpactSignificance.NEGATIVE ||
  ...
  const filteredSupplements = supplements.filter(supplement => {
    ...
    if (searchQuery && !supplement.name.toLowerCase().includes(searchQuery.toLowerCase())) {
      return false;
    }
    ...
    return [...filteredSupplements].sort((a, b) => {
      switch (sortOption) {
        ...
        case SortOption.IMPACT_LOW: {
          ...
          const correlationsA = correlations.filter(c => c.supplement_id === a.id);
          const correlationsB = correlations.filter(c => c.supplement_id === b.id);

          if (correlationsA.length === 0 && correlationsB.length === 0) return 0;
          if (correlationsA.length === 0) return sortOption === SortOption.IMPACT_HIGH ? 1 : -1;
          if (correlationsB.length === 0) return sortOption === SortOption.IMPACT_HIGH ? -1 : 1;
          ...
          const reactionTimeImpactA = latestCorrelationA.reaction_time_impact ?? 0;
          const accuracyImpactA = latestCorrelationA.accuracy_impact ?? 0;

          const scoreImpactB = latestCorrelationB.score_impact ?? 0;
          const reactionTimeImpactB = latestCorrelationB.reaction_time_impact ?? 0;
          ...
          // Calculate overall impact (simple average)
          const overallImpactA = (scoreImpactA + normalizedReactionTimeImpactA + accuracyImpactA) / 3;
          const overallImpactB = (scoreImpactB + normalizedReactionTimeImpactB + accuracyImpactB) / 3;
        ...
        default:
          return new Date(b.intake_time).getTime() - new Date(a.intake_time).getTime();
      }
    });
  }, [filteredSupplements, sortOption, correlations]);

  // Get the selected supplement
  const selectedSupplement = supplements.find(s => s.id === selectedSupplementId);
  ...
  return (
    <div className="container py-8 md:py-12 max-w-screen-xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Supplement Effectiveness Reports</h1>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <Select value={sortOption} onValueChange={(value) => setSortOption(value as SortOption)}>
                    ...
                    <SelectContent>
                      <SelectItem value={SortOption.RECENT}>Most Recent</SelectItem>
                      <SelectItem value={SortOption.NAME_ASC}>Name (A-Z)</SelectItem>
                      <SelectItem value={SortOption.NAME_DESC}>Name (Z-A)</SelectItem>
                      <SelectItem value={SortOption.IMPACT_HIGH}>Highest Impact</SelectItem>
                      <SelectItem value={SortOption.IMPACT_LOW}>Lowest Impact</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                    {sortedSupplements.map(supplement => {
                      ...
                      return (
                        <Button
                          key={supplement.id}
                          variant={selectedSupplementId === supplement.id ? "default" : "outline"}
        ...
        <div className="md:col-span-2">
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-muted/20 rounded-lg">
              ...
              <h2 className="text-xl font-semibold mb-2">No Supplement Selected</h2>
              <p className="text-muted-foreground mb-6">
                Select a supplement from the list to view its effectiveness report
              </p>
            </div>
          ) : (
            <SupplementEffectivenessReport
              ...
              testResults={testResults.filter(result =>
                new Date(result.date) >= new Date(selectedSupplement.intake_time)
              )}
              onExport={() => {
                toast({
