...
export function calculateReactionTestResults(
  ...
  const validTrials = trials.filter(
    (t) => t.correct && t.reactionTime !== null,
  );
  ...
  if (avgReactionTime <= excellentTime) {
    ...
    reactionTimeScore =
      90 -
      ((avgReactionTime - excellentTime) / (goodTime - excellentTime)) * 20;
  } else if (avgReactionTime <= averageTime) {
    reactionTimeScore =
      70 - ((avgReactionTime - goodTime) / (averageTime - goodTime)) * 20;
  } else if (avgReactionTime <= slowTime) {
    reactionTimeScore =
      50 - ((avgReactionTime - averageTime) / (slowTime - averageTime)) * 30;
  } else {
    reactionTimeScore = Math.max(
      10,
      20 - ((avgReactionTime - slowTime) / 100) * 2,
    );
  }
  ...
  const finalScore = Math.round(
    reactionTimeScore * 0.6 + accuracyPercentage * 0.4 - tooEarlyPenalty,
  );
