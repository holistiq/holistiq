...
export function generateOnOffComparisonData(
  ...
  testResults.forEach((test) => {
    const { periodType, supplementId: testSupplementId } =
      determineTestPeriodType(test, periods);

    if (
      periodType === PeriodType.SUPPLEMENT &&
      testSupplementId === supplementId
    ) {
      onPeriodTests.push(test);
    } else if (
      periodType === PeriodType.BASELINE ||
      periodType === PeriodType.WASHOUT
    ) {
...
export function generateBeforeAfterComparisonData(
  ...
  testResults.forEach((test) => {
    ...
    const daysDifference =
      (testDate.getTime() - firstIntakeDate.getTime()) / (1000 * 60 * 60 * 24);
...
export function calculateComparisonMetrics(
  ...
  const baselineScoreAvg =
    baselineData.reduce((sum, test) => sum + test.score, 0) /
    baselineData.length;
  const baselineReactionTimeAvg =
    baselineData.reduce((sum, test) => sum + test.reactionTime, 0) /
    baselineData.length;
  const baselineAccuracyAvg =
    baselineData.reduce((sum, test) => sum + test.accuracy, 0) /
    baselineData.length;

  // Calculate averages for comparison data
  const comparisonScoreAvg =
    comparisonData.reduce((sum, test) => sum + test.score, 0) /
    comparisonData.length;
  const comparisonReactionTimeAvg =
    comparisonData.reduce((sum, test) => sum + test.reactionTime, 0) /
    comparisonData.length;
  const comparisonAccuracyAvg =
    comparisonData.reduce((sum, test) => sum + test.accuracy, 0) /
    comparisonData.length;

  // Calculate percentage changes
  const scoreChange =
    ((comparisonScoreAvg - baselineScoreAvg) / baselineScoreAvg) * 100;
  const reactionTimeChange =
    ((baselineReactionTimeAvg - comparisonReactionTimeAvg) /
      baselineReactionTimeAvg) *
    100; // Inverted for reaction time (lower is better)
  const accuracyChange =
    ((comparisonAccuracyAvg - baselineAccuracyAvg) / baselineAccuracyAvg) * 100;
  ...
  const isSignificant =
    ...
    (Math.abs(scoreChange) > 10 ||
      Math.abs(reactionTimeChange) > 10 ||
