...
if (!componentPath) {
  console.error(
    `${colors.red}Error: Component path is required${colors.reset}`,
  );
  showHelp();
  process.exit(1);
}
...
if (!fs.existsSync(absoluteComponentPath)) {
  console.error(
    `${colors.red}Error: Component file not found: ${absoluteComponentPath}${colors.reset}`,
  );
  process.exit(1);
}
...
console.log(
  `${colors.bright}Analyzing usage of component: ${colors.cyan}${componentName}${colors.reset} (${componentPath})\n`,
);
...
function analyzeFile(filePath) {
  try {
    ...
    const importRegex = new RegExp(
      `import\\s+{[^}]*\\b${componentName}\\b[^}]*}\\s+from|import\\s+${componentName}\\s+from`,
    );
    ...
    const jsxRegex = new RegExp(
      `<${componentName}\\s|<${componentName}>|<${componentName}\\s*\\/>`,
    );
    ...
    console.error(
      `${colors.red}Error analyzing file ${filePath}: ${error.message}${colors.reset}`,
    );
  }
}
...
function displayResults() {
    ...
    imports.forEach((item, index) => {
      ...
      if (verbose) {
        item.importLines.forEach((line) => {
          console.log(
            `   ${colors.dim}Line ${line.lineNumber}:${colors.reset} ${line.content}`,
          );
        });
      }
      ...
      if (item.hasJsxUsage) {
        console.log(
          `   ${colors.green}✓ Component is used in JSX${colors.reset}`,
        );
        ...
        console.log(
          `   ${colors.yellow}⚠ Component is imported but no JSX usage found${colors.reset}`,
        );
      }
    });
  }
  ...
  if (activelyUsed) {
    ...
    console.log(
      `${colors.yellow}Consider checking the import locations to verify usage.${colors.reset}`,
    );
  } else {
    console.log(
    ...
    console.log(
      `${colors.yellow}It might be used through dynamic imports or string references.${colors.reset}`,
    );
  }
}
